<template>
  <div class="left-panel">
    <button @click="$emit('toggle-panel')" class="toggle-btn">닫기</button>
    <h2>찜목록</h2>
    <ul>
      <li>장소 1</li>
      <li>장소 2</li>
    ...
    <div v-if="selectedProperty && selectedProperty.length">
      <h2>아파트명: {{ selectedProperty[0].apartName }}</h2>
      <h4>도로명 주소: {{ selectedProperty[0].roadName }}</h4>
      <h5>건축년도: {{ selectedProperty[0].buildingYear }}</h5>
