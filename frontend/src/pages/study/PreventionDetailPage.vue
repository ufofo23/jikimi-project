...
import api from '@/api/preventionApi';
...
// 목록으로 돌아가는 함수
const back = () => {
  router.push({ name: 'preventionListPage' }); // 올바른 라우트 이름을 사용하여 이동
};
...
<template>
  <div class="container d-flex flex-column min-vh-100">
    <div class="flex-grow-1">
      <h1 class="mt-4 text-center">{{ article.preventionTitle }}</h1>
      ...
      <div v-else class="content my-4">{{ article.preventionContent }}</div>
    </div>
